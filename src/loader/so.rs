...
impl Loader for ShareOnline {
    ...
    fn check(&self, url: &str) -> Result<Option<DownloadFile>> {
        ...
        let link: &str = res
            .get(0)
            .ok_or("Can't find the share-online host in the link")?;
        ...
        f_info.hoster = FileHoster::ShareOnline;
        f_info.name = String::from(
            &Regex::new(r"NAME: ([^\s]+)")?
                .find(&body)
                .ok_or("No name available")?
                .as_str()[6..],
        );
        f_info.url = url.to_string();
        f_info.size = Regex::new(r"SIZE: ([^\s]+)")?
            .find(&body)
            .ok_or("No size available")?
            .as_str()[6..]
            .parse::<usize>()?;
        f_info.hash = FileHash::Md5(String::from(
            &Regex::new(r"MD5: ([^\s]+)")?
                .find(&body)
